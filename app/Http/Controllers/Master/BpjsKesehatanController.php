...
class BpjsKesehatanController extends Controller
{
    ...
    {
        return view('master.bpjs-kesehatan');
    }

    public function list_data(Request $request)
    {
        $columns = array(
            0 => 'mst_bpjs_kesehatan.id',
            1 => 'mst_bpjs_kesehatan.id',
            2 => 'mst_karyawan.nama',
            3 => 'mst_devisi.nama',
            4 => 'mst_bpjs_kesehatan.nilai',
            5 => 'mst_bpjs_kesehatan.id',
        );

        $totalData = DB::table('mst_bpjs_kesehatan')
            ->rightjoin('mst_karyawan', 'mst_bpjs_kesehatan.id_karyawan', '=', 'mst_karyawan.id')
        ...
        if (empty($request->input('search.value'))) {
            $posts = DB::table('mst_bpjs_kesehatan')
                ->rightjoin('mst_karyawan', 'mst_bpjs_kesehatan.id_karyawan', '=', 'mst_karyawan.id')
                ->join('mst_devisi', 'mst_karyawan.id_devisi', '=', 'mst_devisi.id')
                ->selectraw('mst_bpjs_kesehatan.*, mst_karyawan.nama as nama_karyawan, mst_devisi.nama as nama_devisi, mst_karyawan.id as id_kry')
                ->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
            ...
            $totalFiltered = DB::table('qview_devisi')
                ->rightjoin('mst_karyawan', 'mst_bpjs_kesehatan.id_karyawan', '=', 'mst_karyawan.id')
                ->join('mst_devisi', 'mst_karyawan.id_devisi', '=', 'mst_devisi.id')
                ->selectraw('mst_bpjs_kesehatan.*, mst_karyawan.nama as nama_karyawan, mst_devisi.nama as nama_devisi')
                ->where('nama_karyawan', 'LIKE', "%{$search}%")
                ->orWhere('nama_devisi', 'LIKE', "%{$search}%")
                ->count();
        }
    ...
    public function update(Request $request)
    {
        ...
        BpjsKesehatanModel::where('id', $request->sysid)->update($data);
