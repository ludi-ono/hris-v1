...
class MasterKaryawanController extends Controller
{
    ...
    public function list_data(Request $request)
    {
        ...
        if (!empty($posts)) {
            foreach ($posts as $post) {
                ...
                $nestedData['tanggal_masuk'] = $post->tanggal_masuk;
                $nestedData['action'] = "&emsp;<a href='javascript:void(0)' id='edit' data-toggle='tooltip' title='Edit' data-id='$post->id' data-original-title='' class='Edit btn btn-warning btn-sm'><i class='fas fa-pencil-alt'></i> &nbsp; Edit </a>
                                        ...
                                        <a href='javascript:void(0)' id='detail' data-toggle='tooltip' title='Detail' data-id='$post->id' data-original-title='' class='Detail btn btn-primary btn-sm'><i class='fa-regular fa-file-pdf'></i> &nbsp; Detail </a>";
    ...
    public function store(Request $request)
    {
        ...
        if ($request->file('file_foto')) {
            ...
            $fileFoto       = 'F-' . uniqid() . '.' . $file_foto->getClientOriginalExtension();
            $file_foto->move(public_path('/images/foto/'), $fileFoto);
        } else {
            $fileFoto = NULL;
        }
    ...
    public function update(Request $request)
    {
        ...
        if ($request->file('file_foto')) {
            ...
            $fileFoto       = 'F-' . uniqid() . '.' . $file_foto->getClientOriginalExtension();
            $file_foto->move(public_path('/images/foto/'), $fileFoto);
            MasterKaryawanModel::where('id', $request->sysid)->update(['file_foto' => $fileFoto]);
        }
        ...
        // $data_kry = MasterKaryawanModel::where('id', $request->sysid)->first();
