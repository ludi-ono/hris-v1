...
class KaryawanSakitController extends Controller
{
    ...
    {
        return view('karyawan.karyawan-sakit');
    }
    ...
    public function list_data(Request $request)
    {
        ...
        if (empty($request->input('search.value'))) {
            ...
            $posts =  DB::table('mst_tidakhadir')
                ...
                ->offset($start)
                ->limit($limit)
                ->where([['jenis_ijin', '=', 'Sakit'],['mst_tidakhadir.id_karyawan', '=', $id_kry],['mulai', 'LIKE', "%{$search}%"]])
                ->orWhere([['jenis_ijin', '=', 'Sakit'],['mst_tidakhadir.id_karyawan', '=', $id_kry],['sampai', 'LIKE', "%{$search}%"]])
                ->orWhere([['jenis_ijin', '=', 'Sakit'],['mst_tidakhadir.id_karyawan', '=', $id_kry],['status', 'LIKE', "%{$search}%"]])
                ->orWhere([['jenis_ijin', '=', 'Sakit'],['mst_tidakhadir.id_karyawan', '=', $id_kry],['keterangan', 'LIKE', "%{$search}%"]])
                ->orWhere([['jenis_ijin', '=', 'Sakit'],['mst_tidakhadir.id_karyawan', '=', $id_kry],['no_dokumen', 'LIKE', "%{$search}%"]])
                ->orderBy($order, $dir)
                ->get();
    ...
    public function store(Request $request)
    {
        ...
        $year = date('Y');
        $month = date('m');
        $no = TidakHadirModel::where('jenis_ijin','Sakit')->where('no_dokumen', 'LIKE', 'S-'.substr($year,-2)."%")->count();
        $docno = 'S-'.substr($year,-2).$month.substr('0000'.$no+1,-4);
        ...
        $fileName       = 'S-' . uniqid() . '.' . $file_upload->getClientOriginalExtension();
