$(document).ready(function () {
        ...
        $("#tbl_presensi_hdr").DataTable({
            ...
            columns: [{
                ...
                }, // 4
                {
                    ...
                    render:function(d){
                        if(d == 0){
                            ket = 'Lembur Belum di Generate Untuk Gaji';
                            return '<span class="badge badge-danger">'+ket+'</span>'
                            
                        }else{
                            ket = 'Lembur Sudah di Generate Untuk Gaji';
    ...
    function list_data_dtl(id_hdr) {
        $("#tbl_presensi_detail").DataTable({
            ...
            ajax: {
                url: "/generate-lembur/list-data-dtl",
                type: "GET",
                data: {id_hdr:id_hdr},
            },
            columns: [{
    ...
    function list_data_tmp() {
        $("#tbl_presensi_dtl").DataTable({
            ...
            ajax: {
                url: "/generate-lembur/list-data-tmp",
                type: "GET",
            },
            columns: [{
                    data: "id",
    ...
    function save_data(state) {
        // if (state == 'ADD') {
            url_save = '/generate-lembur/store';
        // } else {
    ...
    $('#batal, #back').click(function(event) {
        $.ajax({
            url: "/generate-lembur/destroy-tmp",
            type: 'POST',
        }).done(function() {
            $('#list_data').show('slow');
            $('#add_data').hide('slow');
    ...
    $('body').on('click', '#delete', function (e) {
        ...
        swal({
            ...
            if (Delete) {
                $.ajax({
                    type: "post",
                    url: "/generate-lembur/destroy/" + id,
                    success: function (response) {
                        for (var key in response) {
                            var flag = response["success"];
                        }
