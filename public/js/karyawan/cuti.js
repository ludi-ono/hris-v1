$(document).ready(function() {
    ...
    function save_data(state) {
        if (state == 'ADD') {
            url_save = '/karyawan-cuti/store';
        } else {
            url_save = '/karyawan-cuti/update';
        }
    ...
    function form_state(state) {
        switch (state) {
            ...
            case 'ADD_HDR':
                reset_input();
                $("#state").val("ADD");
                $('#title_input').html('Input Data Cuti');
                $('#add_data').show('slow');
                $('#list_data').hide('slow');
                break;
            case 'SAVE_HDR':
            ...
            case 'EDIT_HDR':
                $("#state").val("EDIT");
                $('#title_input').html('Edit Data Cuti');
                $('#add_data').show('slow');
                $('#list_data').hide('slow');
                break;
            case 'REVISI_HDR':
    ...
    $('#save').click(function (event) {
        ...
        if ($('#mulai').val().length == 0) {
            ...
            message = 'Tanggal Sampai Cuti Belum Diisi';
        } else {
            message = '';
        }
    ...
    $('body').on('click', '#detail', function (e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#keterangan_d').val(response.keterangan);
                $('#file_show').val(response.filename);
                // $('#file_show').attr("src", "images/card-front.jpg");
                // file_show = $('#file_show');
                // file_show.load(url+'/file_upload/'+response.filename);
            }
        }).done(function(data){
            $('#list_data').hide('slow');
