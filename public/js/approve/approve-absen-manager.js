$(document).ready(function () {
    ...
    function list_data() {
        $("#tbl_approve").DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "/approve-absen-manager/list-data-absen",
                type: "GET",
            },
            columns: [
                ...
                { data: "jenis_ijin", name: "jenis_ijin", visible: true }, // 2
                { data: "mulai", name: "mulai", visible: true }, // 2
                { data: "sampai", name: "sampai", visible: true }, // 2
                { data: "jumlah_hari", name: "jumlah_hari", visible: true }, // 2
                { data: "filename", name: "filename", visible: true }, // 2
    ...
    $('body').on('click', '#approve', function (e) {
        ...
        swal({
            ...
            if (Delete) {
                $.ajax({
                    type: "post",
                    url: "/approve-absen-manager/approve-absen/" + id,
    ...
    $('body').on('click', '#reject', function (e) {
        ...
        swal({
            ...
            if (Delete) {
                $.ajax({
                    type: "post",
                    url: "/approve-absen-manager/reject-absen/" + id,
                    success: function (response) {
                        for (var key in response) {
                            var flag = response["success"];
                        }
